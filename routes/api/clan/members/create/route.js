...
const route = {
	...
	run: async (url, request, response, discord, pg, token) => {
		if (request.method.toUpperCase() !== 'POST' || !token || !url.searchParams.has('username') || !url.searchParams.has('user_id') || !url.searchParams.has('joined_at') || !url.searchParams.has('rank')) {
