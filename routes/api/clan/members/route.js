		...
		switch (request.method) {
			case 'GET': {
				Util.database.query('SELECT * FROM clan_members ORDER BY joined_at ASC')
					.then(async res => {
						const data = await Promise.all(res.rows.map(async member => {
			...
			case 'POST': {
				if (
					!url.searchParams.has('username') || !validateUsername(url.searchParams.get('username')) ||
					!url.searchParams.has('joined_at') || !validateJoinedDate(url.searchParams.get('joined_at')) ||
					(url.searchParams.has('user_id') && !validateId(url.searchParams.get('user_id'))) ||
					!url.searchParams.has('rank') || !validateRank(url.searchParams.get('rank'))
				) {
				...
				if (!member || !member.discord.roles.cache.some(r => r.id === Util.config.LEADER_ROLE_ID || r.id === Util.config.CO_LEADER_ROLE_ID)) {
				...
				await Util.database.query(
					'INSERT INTO clan_members VALUES ($1, $2, $3, $4)',
					[
			...
			case 'PATCH': {
				if (
					...
					(url.searchParams.has('user_id') &&	!validateId(url.searchParams.get('user_id'))) ||
					!url.searchParams.has('rank') || !validateRank(url.searchParams.get('rank')) ||
					!url.searchParams.has('member') || !validateUsername(url.searchParams.get('member'))
				) {
					response.writeHead(400, { 'Content-Type': 'application/json' });
					response.write(JSON.stringify({
						status: 400,
				...
				await Util.database.query(
					'UPDATE clan_members SET username = $1, user_id = $2, joined_at = $3, rank = $4 WHERE username = $5',
					[
						url.searchParams.get('username'),
						url.searchParams.get('user_id') ? url.searchParams.get('user_id') : null,
			...
			case 'DELETE': {
				...
				await Util.database.query(
					'DELETE FROM clan_members WHERE username = $1',
					[ url.searchParams.get('member') ]
				);
...
function sendUpdate(author, method, searchParams) {
	Util.discord.channels.cache.get("881512057822933044").send(
		`__Dashboard updated:__
		 ...
		 - **Method:** \`${method}\`
		 - **Member:** \`${searchParams.get('member')}\`${method !== 'DELETE' ? `
