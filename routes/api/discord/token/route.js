...
const route = {
	...
	run: async (url, request, response, discord, pg) => {
		if (request.method.toUpperCase() !== 'POST' || !url.searchParams.get('code') || !url.searchParams.get('user_token')) {
		...
		if (!tokens.length) {
			await pg.query(`INSERT INTO web_clients VALUES ('${user.data.id}', '${res.data.access_token}', '{ "${url.searchParams.get('user_token')}" }', '${res.data.refresh_token}', '${new Date(Date.now() + res.data.expires_in * 1000).toISOString()}')`);
		} else {
			const new_mayze_tokens = tokens[0].mayze_tokens;
			new_mayze_tokens.push(url.searchParams.get('user_token'));
		...
		// setTimeout(() => refreshDiscordToken(pg, url.searchParams.get('user_token'), res.data.refresh_token), res.data.expires_in - 3600000);
